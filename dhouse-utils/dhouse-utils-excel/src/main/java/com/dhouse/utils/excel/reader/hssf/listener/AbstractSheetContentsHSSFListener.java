package com.dhouse.utils.excel.reader.hssf.listener;
...
public abstract class AbstractSheetContentsHSSFListener implements HSSFListener {
    ...
    public AbstractSheetContentsHSSFListener(ContentReader contentReader) {
    ...
    /**
     * CellRecord记录的处理
     * 调用ContentReader的cell方法，处理单元格内容
     * @param cellValue
     */
    private void cellHandle(String cellValue) {
        //此处s字段，将列号+行号合成如A1的字符串，适配xlsx的格式
        contentReader.cell(ExcelUtil.excelColIndexToStr(currentColumnIndex)+(currentRowIndex+1),cellValue,null);
    }
    /**
     * LabelSSTRecord记录的处理
     * 字符串标签记录
    ...
    private void labelSSTHandle(LabelSSTRecord record) {
        if (sstRecord == null) {
            throw new ExcelException(DefaultExceptionInfo.defaultExceptionInfo.ANALYSIS_ERROR_XLS_NO_SST_RECORD);
        } else {
    ...
    /**
     * CellRecord记录的处理（开始）
     * 开始新行时调用ContentReader的startRow方法，设置当前列号
     * @param record
     */
    private void cellHandlePre(CellRecord record) {
        if(currentRowIndex != record.getRow()){
            ...
            contentReader.startRow(currentRowIndex);
        }
        currentColumnIndex = record.getColumn();
    }

    /**
     * 行结束时调用contentReader的endRow方法，重置当前列
     * @param record
     */
    private void endRow(LastCellOfRowDummyRecord record) {
        //行结束时调用
        contentReader.endRow(currentRowIndex);
        //行结束，重置当前列
